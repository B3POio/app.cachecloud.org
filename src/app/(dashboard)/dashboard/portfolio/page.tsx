...
// ---- API wrappers to your Next.js proxy at /api/crypto/portfolio ----
async function apiGetWallets(chain?: Chain) {
  const url = `/api/crypto/portfolio${chain ? `?chain=${chain}` : ""}`;
...
async function apiDeleteWallet(chain: Chain, address: string) {
  const res = await fetch(
    `/api/crypto/portfolio?chain=${chain}&address=${encodeURIComponent(address)}`,
...
export default function PortfolioPage() {
  ...
  // ---------- Load from backend (server derives user from session) ----------
  ...
  async function saveEdit(id: string) {
    ...
    try {
      ...
      await refreshFromApi(); // get truth from backend
      cancelEdit();
    } catch (e: any) {
      console.error(e);
    ...
    <div className="h-full w-full p-4 md:p-6">
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-lg bg-primary px-3 py-2 text-sm font-medium text-primary-foreground shadow hover:bg-primary/90"
        >
      ...
      {wallets.length === 0 ? (
        <div className="flex h-[60vh] items-center justify-center">
          <div className="max-w-md text-center">
            ...
            <button
              onClick={() => setOpen(true)}
              className="flex items-center justify-center gap-2 rounded-lg bg-primary px-3 py-2 text-sm font-medium text-primary-foreground shadow hover:bg-primary/90"
            >
              <Plus className="h-4 w-4" />
              <span className="hidden sm:inline">Add wallet</span>
            </button>
          ...
          {wallets.map((w) => (
            <div key={w.id} className="rounded-2xl border p-4">
              <div className="flex items-center justify-between gap-3">
                ...
                {/* Right: actions (icons on mobile, text on sm+) */}
